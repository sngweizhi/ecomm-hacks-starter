...
/**
 * Create a manual listing with photos and schedule background collage generation
 * This action handles:
 * 1. Uploading photos to storage
 * 2. Creating an active listing immediately with uploaded photos
 * 3. Scheduling a background job to generate 3x3 collage and split it into 9 images
 */
export const createManualListing = action({
  args: {
    title: v.string(),
    description: v.string(),
    price: v.number(),
    ...
    photos: v.array(v.string()), // Array of base64 image strings (data URIs or base64)
  },
  returns: v.object({
    success: v.boolean(),
  ...
  handler: async (ctx, args): Promise<{
    ...
    try {
      // Convert photo URIs to base64 and upload to storage
      const uploadedImageUrls: string[] = []
      const storedImagesForPanel: Array<{ imageBase64: string; description: string }> = []

      for (const photo of args.photos) {
        // Handle both data URIs (data:image/jpeg;base64,...) and plain base64
        let base64Data = photo
        if (photo.startsWith("data:")) {
          const base64Match = photo.match(/base64,(.+)$/)
          if (base64Match) {
        ...
        // Convert base64 to Uint8Array (Convex-compatible, no Buffer needed)
        ...
        storedImagesForPanel.push({
          imageBase64: base64Data,
          description: `${args.title} - photo ${uploadedImageUrls.length}`,
        })
      }

      // Create active listing immediately with uploaded photos
      const listingId: Id<"listings"> = await ctx.runMutation(internal.listings.createManualActiveListing, {
        ownerId: userId,
        title: args.title,
        description: args.description,
        price: args.price,
      ...
      // Schedule background job to generate collage and split it
      await ctx.scheduler.runAfter(0, internal.listingsCollageProcessing.processManualListingCollage, {
        listingId,
        images: storedImagesForPanel,
        title: args.title,
        description: args.description,
        category: args.category,
      })
      ...
      console.error("[Listings] Manual listing creation error:", error)
      return {
        success: false,
 ...
 * Removes RAG embeddings and related favorites before deleting the record
 */
export const deleteListing = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Remove embedding first so we still have the entryId
    if (listing.ragEntryId) {
      await ctx.scheduler.runAfter(0, internal.listingEmbeddings.removeListingEmbedding, {
        listingId: args.id,
        ragEntryId: listing.ragEntryId,
    ...
    // Clean up favorites referencing this listing
...
/**
 * Internal mutation to update listing images and mark processing as completed
 */
export const updateListingImages = internalMutation({
  args: {
...
/**
 * Internal mutation to create a manual draft listing
 */
export const createManualDraftListing = internalMutation({
  args: {
    ownerId: v.string(),
    title: v.string(),
    description: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    // Build extended description with condition
    ...
    const fullDescription = `Condition: ${conditionLabels[args.condition]}\n\n${args.description}`
...
/**
 * Internal mutation to create an active manual listing (for immediate display)
 */
export const createManualActiveListing = internalMutation({
  args: {
    ownerId: v.string(),
    title: v.string(),
    description: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const listingId = await ctx.db.insert("listings", {
      ...
      favoriteCount: 0,
      searchText: `${args.title} ${fullDescription}`.toLowerCase(),
      processingStatus: "processing", // Will be updated to "completed" after collage processing
    })

    // Schedule embedding generation for active listing
    await ctx.scheduler.runAfter(0, internal.listingEmbeddings.embedListing, {
      listingId,
    })
